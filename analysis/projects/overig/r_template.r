...
## KRUSKAL-WALLIS TEST
kruskal_wallis <- kruskal.test(y_var ~ x_var,
                              data = df)

kruskal_wallis

# POST HOC
kruskal_wallis_dunn <- dunn.test(y_var,
                                 g = x_var,
                                 method = "bonferroni",
                                 list = TRUE)
...
# Dunn post hoc tests requires a minimum of 3 values per category in x variable
